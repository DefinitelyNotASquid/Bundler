...
public class ProjectSettings implements ProjectComponent {
  ...
  private final BundlerSettingsPersistent bundlerSettingsPersistent;
  ...
  public ProjectSettings(Project project, ProgressManager progressManager) {
    ...
    this.bundlerSettingsPersistent = BundlerSettingsPersistent
        .getInstance(project);
    updateExecutable();
  ...
  private void updateExecutable() {
    if (bundlerSettingsPersistent == null) {
      executable = null;
    } else {
      executable = bundlerSettingsPersistent.bundlerLocation;
    }
  }
  ...
  private void runAutomaticExecutableRetrieval() {
    ...
    app.executeOnPooledThread(() -> app.invokeLater(() -> {
      try {
        Path bundlerNodeExecutableLocation = determineAndSetExecutable("node");
        if (bundlerNodeExecutableLocation != null) {
          executable = bundlerNodeExecutableLocation.toString();
        }
